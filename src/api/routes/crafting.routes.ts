 ...
 */
router.get('/materials', verifyToken, async (req: Request, res: Response) => {
  try {
    ...
    const materials = materialsResult.rows.map((row: any) => ({
 ...
 */
router.post('/craft', verifyToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
 ...
 */
router.get('/gear', verifyToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
    ...
    const gear = gearResult.rows.map((row: any) => ({
      id: row.id,
      name: row.name,
      slot: row.slot,
 ...
 * Equip a gear piece
 */
router.post('/equip/:gearId', verifyToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
