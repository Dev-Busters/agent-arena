...
class MatchmakingQueue {
  private queue: Map<string, QueueEntry> = new Map(); // user_id -> entry
  private tickInterval: NodeJS.Timeout | null = null;
  private readonly SKILL_RANGE = 150; // +/- rating range for matching
  private readonly QUEUE_TIMEOUT = 30000; // 30 seconds before expanding range

  /**
   * Add player to queue
   */
  async addToQueue(userId: string, agentId: string, rating: number): Promise<void> {
  ...
  private findMatch(entry: QueueEntry): QueueEntry | null {
    ...
    for (const [otherUserId, otherEntry] of this.queue.entries()) {
      if (otherUserId === entry.user_id) continue; // Skip self
   ...
   */
  async tryMatchPlayers(): Promise<Array<{ player1: QueueEntry; player2: QueueEntry }>> {
    ...
    for (const entry of queueArray) {
      ...
      if (opponent) {
        ...
        console.log(
          `🎮 Match found: ${entry.user_id} (${entry.rating}) vs ${opponent.user_id} (${opponent.rating})`
        );
      }
    }
   ...
   */
  startMatchmaking(tickIntervalMs: number = 5000): void {
 ...
 */
export async function updatePlayerRating(userId: string, ratingChange: number): Promise<void> {
  await pool.query(
