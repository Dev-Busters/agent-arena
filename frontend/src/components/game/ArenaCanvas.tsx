...
export default function ArenaCanvas({
  ...
  useEffect(() => {
    ...
    const combatManager = new CombatManager({
      ...
      onAshEarned:     (v) => { runManager.onAshEarned(v); },
      onEmberEarned:   (v) => { runManager.onEmberEarned(v); },
      onFragmentEarned:(d, v) => { runManager.onFragmentEarned(d, v); },
      onMaterialsDropped:(stacks) => { runManager.onMaterialsDropped(stacks); },
      onGearDropped:(item) => { runManager.onGearDropped(item); },
      onArenaMarkEarned:(n) => { runManager.onArenaMarkEarned(n); },
      getCurrentFloor: () => runManager.getFloor(),
      particles,
      sound,
