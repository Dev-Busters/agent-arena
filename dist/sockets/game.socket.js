    ...
    io.on('connection', (socket) => {
        ...
        socket.on('join_queue', async (data, callback) => {
            try {
                ...
                // Get agent
                const agentResult = await pool.query('SELECT * FROM agents WHERE id = $1 AND user_id = $2', [data.agent_id, userId]);
        ...
        socket.on('start_battle', async (data, callback) => {
            try {
                // Get both agents
                const agents = await pool.query('SELECT * FROM agents WHERE id = ANY($1)', [[data.agent1_id, data.agent2_id]]);
