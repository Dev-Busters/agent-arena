...
export function setupDungeonSockets(io) {
    io.on("connection", (socket) => {
         ...
         */
        socket.on("start_dungeon", async (payload) => {
            try {
                console.log('🎮 [DUNGEON] start_dungeon triggered:', { userId: payload.userId, agentId: payload.agentId });
                const { userId, agentId } = payload;
                // Fetch agent stats
                const agentResult = await query("SELECT * FROM agents WHERE id = $1 AND user_id = $2", [agentId, userId]);
         ...
         */
        socket.on("enter_room", async (payload) => {
            try {
                ...
                const rng = SeededRandom(`${session.dungeonId}-room-${roomId}`.toString());
        ...
        socket.on("dungeon_action", async (payload) => {
            try {
                ...
                if (session.currentEnemies.length === 0) {
                    ...
                    const difficulty = getDifficultyForFloor(session.depth);
                    const loot = generateLoot(baseGold, baseXp, difficulty, session.depth, rng);
                    // Calculate level up
        ...
        socket.on("next_floor", async (_payload) => {
            try {
                ...
                if (session.depth >= 10) {
                    ...
                    const difficulty = getDifficultyForFloor(session.depth);
                    const seed = Math.floor(Math.random() * 1000000);
                    const map = generateDungeon(seed, difficulty, session.depth, session.playerHp);
         ...
         */
        socket.on("choose_path", async (payload) => {
            try {
                const session = activeDungeonSessions.get(socket.id);
                if (!session) {
                    socket.emit("dungeon_error", { message: "No active dungeon" });
        ...
        socket.on("abandon_dungeon", async (_payload) => {
            try {
                ...
                await query("UPDATE dungeons SET abandoned_at = CURRENT_TIMESTAMP WHERE id = $1", [session.dungeonId]);
                activeDungeonSessions.delete(socket.id);
...
//# sourceMappingURL=dungeon.socket.js.map
