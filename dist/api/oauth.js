...
export async function findOrCreateOAuthUser(profile) {
    ...
    const existingOAuth = await pool.query(`SELECT u.* FROM users u
     JOIN user_oauth_accounts oa ON u.id = oa.user_id
     WHERE oa.provider = $1 AND oa.provider_id = $2`, [profile.provider, profile.providerId]);
    ...
    const existingEmail = await pool.query('SELECT * FROM users WHERE email = $1 AND deleted_at IS NULL', [profile.email]);
    let user;
    let isNew = false;
    if (existingEmail.rows.length > 0) {
        ...
        try {
            const result = await pool.query(`INSERT INTO users (email, username, password_hash, gold)
         VALUES ($1, $2, $3, $4)
         RETURNING *`, [profile.email, username, 'OAUTH_USER', 1000]);
            user = result.rows[0];
            isNew = true;
        }
        catch (err) {
    ...
    // Link OAuth account
    await pool.query(`INSERT INTO user_oauth_accounts (user_id, provider, provider_id, avatar_url)
     VALUES ($1, $2, $3, $4)
     ON CONFLICT (provider, provider_id) DO UPDATE SET avatar_url = $4`, [user.id, profile.provider, profile.providerId, profile.avatar]);
    return { user, isNew };
}
/**
...
export function generateOAuthTokens(user) {
    ...
    }, process.env.JWT_SECRET || 'dev-secret-change-in-production', { expiresIn: '24h' });
    const refreshToken = jwt.sign({ id: user.id, type: 'refresh' }, process.env.JWT_SECRET || 'dev-secret-change-in-production', { expiresIn: '7d' });
