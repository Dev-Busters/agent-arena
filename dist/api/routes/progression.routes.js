 ...
 */
router.post('/dungeon-rewards', verifyToken, async (req, res) => {
    try {
        ...
        // Calculate new level
        const levelUpData = calculateLevelUp(agent.level, agent.experience, xp);
        // Update agent
        await query(`UPDATE agents 
       SET level = $1, experience = $2, current_hp = max_hp
       WHERE id = $3`, [levelUpData.newLevel, levelUpData.newXp, agentId]);
        // Update user gold
        await query(`UPDATE users 
       SET gold = gold + $1
       WHERE id = $2`, [gold, userId]);
        res.json({
            success: true,
...
router.get('/agent/:agentId', async (req, res) => {
    try {
        ...
        const result = await query('SELECT id, level, experience, current_hp, max_hp FROM agents WHERE id = $1', [agentId]);
