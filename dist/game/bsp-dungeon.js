...
(function (Tile) {
    ...
    Tile[Tile["EXIT"] = 2] = "EXIT";
    Tile[Tile["DOOR"] = 3] = "DOOR";
    Tile[Tile["CORRIDOR"] = 4] = "CORRIDOR";
    Tile[Tile["TRAP"] = 5] = "TRAP";
    Tile[Tile["TREASURE"] = 6] = "TREASURE";
    Tile[Tile["PILLAR"] = 7] = "PILLAR";
    Tile[Tile["WATER"] = 8] = "WATER";
    Tile[Tile["LAVA"] = 9] = "LAVA";
    Tile[Tile["ENTRANCE"] = 10] = "ENTRANCE";
})(Tile || (Tile = {}));
...
export class BSPDungeonGenerator {
    ...
    generate(difficulty, depth, seed) {
        ...
        this.initTiles();
        // Build BSP tree
        const root = this.createBSPNode(1, 1, this.config.width - 2, this.config.height - 2);
    ...
    splitNode(node, depth) {
        ...
        if (node.width <= this.config.minPartitionSize * 2 &&
            node.height <= this.config.minPartitionSize * 2)
            return;
    ...
    assignRoomTypes(difficulty, depth) {
        ...
        if (depth >= 3 && this.rooms.length >= 4) {
            ...
            const largest = candidates.reduce((best, room) => (room.width * room.height > best.width * best.height) ? room : best);
    ...
    placeBossFeatures(room) {
        ...
        if (room.width >= 6 && room.height >= 6) {
            ...
            room.features.push({ type: "pillar", x: room.x + px, y: room.y + py }, { type: "pillar", x: room.x + room.width - px - 1, y: room.y + py }, { type: "pillar", x: room.x + px, y: room.y + room.height - py - 1 }, { type: "pillar", x: room.x + room.width - px - 1, y: room.y + room.height - py - 1 });
        }
        this.addCornerTorches(room);
    }
    ...
    addCornerTorches(room) {
        if (room.width >= 3 && room.height >= 3) {
            room.features.push({ type: "torch", x: room.x, y: room.y }, { type: "torch", x: room.x + room.width - 1, y: room.y }, { type: "torch", x: room.x, y: room.y + room.height - 1 }, { type: "torch", x: room.x + room.width - 1, y: room.y + room.height - 1 });
        }
    }
    // ─── Entrance / Exit ─────────────────────────────────────────────────
    placeEntrance() {
    ...
    addSecretRooms() {
        ...
        for (const corridor of [...this.corridors]) {
            ...
            if (sx < 2 || sy < 2 ||
                ...
                sy + secretH >= this.config.height - 2)
                continue;
 ...
 */
export function generateBSPDungeon(seed, difficulty, depth, _playerLevel) {
 ...
 */
export function bspToLegacyFormat(bspMap) {
    return {
        width: bspMap.width,
 ...
 */
export function getRoomGraph(map) {
    const graph = new Map();
 ...
 */
export function findRoomPath(graph, startId, endId) {
 ...
 */
export function dungeonToASCII(map) {
    const chars = {
        [Tile.WALL]: "█",
        [Tile.FLOOR]: ".",
        [Tile.EXIT]: "▼",
...
//# sourceMappingURL=bsp-dungeon.js.map