 ...
 */
export function decideEnemyAction(aiPattern, state, rng) {
    // Check if should flee
    const currentHpPercent = state.enemyHp / state.enemyMaxHp;
    if (currentHpPercent < aiPattern.fleeThreshold &&
        aiPattern.fleeThreshold > 0) {
        return "flee";
    }
    // Check if should defend (low HP or high incoming damage prediction)
    const defenseThreshold = 0.5;
    const predictedDamage = state.playerAttack -
        state.enemyDefense +
        Math.random() * 10 -
        5;
    if (currentHpPercent < defenseThreshold &&
 ...
 */
export function calculateEnemyDamage(enemyAttack, playerDefense, _playerDefended, critical = false) {
 ...
 */
export function getAIPersonality(behavior) {
    const personalities = {
        aggressive: "Charges forward with fury, attacking relentlessly with raw power.",
        ...
        support: "Coordinates with allies, using defensive and healing abilities.",
