 ...
 */
export function getMaterialsForFloor(floor) {
    return Object.values(MATERIALS).filter(m => m.minFloor <= floor);
}
/**
 * Get drop chance for a material at a specific floor
 */
export function getMaterialDropChance(materialId, floor) {
    const material = MATERIALS[materialId];
    if (!material || material.minFloor > floor)
        return 0;
 ...
 */
export function getMaterialColor(rarity) {
    const colors = {
