...
export function getRarityIndex(r) {
...
const ALL_UNIQUES = [...UNIQUE_ITEMS, ...SET_PIECE_DEFS];
// ============================================================
...
export const ZONE_LOOT_MODIFIERS = {
 ...
 */
export function rollRarity(rng, ctx) {
...
function rollAffixes(rarity, itemLevel, rng, minAffixes, maxAffixesOverride, guaranteedAffixId) {
    ...
    for (let i = affixes.length; i < count; i++) {
        ...
        const eligible = pool.filter(a => a.tier <= maxTier &&
            !usedIds.has(a.id) &&
...
function generateSingleItem(entry, ctx, rng) {
    ...
    const affixes = rollAffixes(rarity, itemLevel, rng, entry.minAffixes, entry.maxAffixes, entry.guaranteedAffix);
    return buildItem(base, rarity, affixes, itemLevel);
}
/**
 * Generate complete loot drops from a loot table + context
 */
export function generateLootFromTable(table, ctx, rng) {
 ...
 */
export function generateLoot(gold, xp, difficulty, depth, rng, enemyType, magicFind, zoneType, isBoss) {
    const ctx = {
        difficulty,
        depth,
 ...
 */
export function calculateLevelUp(currentLevel, currentXp, gainedXp) {
...
export function calculateSetBonuses(equippedItems) {
    ...
    for (const set of ITEM_SETS) {
        const equipped = equippedItems.filter(item => item.setId === set.id || set.pieces.includes(item.id)).length;
 ...
 */
export function getTotalItemStats(items) {
    const total = {};
    const statKeys = [
...
export function getRarityHexColors(rarity) {
    const colors = {
...
export function formatItemStats(stats) {
    const parts = [];
    if (stats.attack)
        parts.push(`+${stats.attack} ATK`);
    if (stats.defense)
        parts.push(`+${stats.defense} DEF`);
    if (stats.speed)
        parts.push(`${stats.speed > 0 ? '+' : ''}${stats.speed} SPD`);
    if (stats.accuracy)
        parts.push(`+${stats.accuracy} ACC`);
    if (stats.evasion)
        parts.push(`+${stats.evasion} EVA`);
    if (stats.hp)
        parts.push(`+${stats.hp} HP`);
    if (stats.critChance)
        parts.push(`+${stats.critChance}% CRIT`);
    if (stats.critDamage)
        parts.push(`+${stats.critDamage}% CDMG`);
    if (stats.lifeSteal)
        parts.push(`+${stats.lifeSteal}% LS`);
    if (stats.thorns)
        parts.push(`+${stats.thorns} THORNS`);
    if (stats.magicFind)
        parts.push(`+${stats.magicFind}% MF`);
    return parts;
}
export function formatItemTooltip(item) {
    ...
    if (item.damageType)
        lines.push(`Damage: ${item.damageType}`);
    if (item.flavorText)
        lines.push(`"${item.flavorText}"`);
