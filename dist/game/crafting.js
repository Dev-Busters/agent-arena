 ...
 */
export function generateCraftedGear(slot, materials, rng) {
 ...
 */
function selectAffix(pool, targetRarity, rng) {
    const rarityMap = {
        common: 1,
        uncommon: 2,
        rare: 3,
 ...
 */
export function getCraftingCost(slot, rarity) {
    const costMap = {
 ...
 */
export function formatGearStats(gear) {
    const parts = [];
    if (gear.totalStats.attack)
        parts.push(`+${gear.totalStats.attack} ATK`);
    if (gear.totalStats.defense)
        parts.push(`+${gear.totalStats.defense} DEF`);
    if (gear.totalStats.speed)
        parts.push(`+${gear.totalStats.speed} SPD`);
    if (gear.totalStats.accuracy)
        parts.push(`+${gear.totalStats.accuracy} ACC`);
    if (gear.totalStats.evasion)
        parts.push(`+${gear.totalStats.evasion} EVA`);
    return parts;
}
