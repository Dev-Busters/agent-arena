 ...
 */
export function tryApplyEffect(targetEffects, effectType, sourceId, currentTurn, targetDefense = 0, bonusChance = 0, bonusStacks = 0, bonusDuration = 0, rng = Math.random) {
    ...
    if (existing) {
        ...
        existing.duration = Math.min(config.maxDuration, Math.max(existing.duration, config.baseDuration + bonusDuration));
        return {
            applied: true,
            resisted: false,
 ...
 */
export function processStatusEffects(effects, maxHp, currentHp) {
    const results = [];
 ...
 */
export function getSpeedModifier(effects) {
 ...
 */
export function getHealingModifier(effects) {
 ...
 */
export function rollAttackEffects(attackerAbilities, targetEffects, sourceId, currentTurn, targetDefense, isCritical, rng = Math.random) {
    const results = [];
    for (const ability of attackerAbilities) {
        // Crits get +15% bonus chance to apply effects
        const critBonus = isCritical ? 0.15 : 0;
        const result = tryApplyEffect(targetEffects, ability.effectType, sourceId, currentTurn, targetDefense, ability.bonusChance + critBonus, ability.bonusStacks, ability.bonusDuration, rng);
 ...
 */
export function removeEffect(effects, type) {
 ...
 */
export function clearAllEffects(effects) {
